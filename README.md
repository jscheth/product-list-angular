...
This is a small Angular application built as a take-home assessment to demonstrate:
...
Product data is currently hard-coded within `ProductService` and exposed as an RxJS stream to simulate a backend API.

This approach was chosen to:

- Keep the assessment deterministic and easy to run  
- Avoid external API dependencies or network failures  
- Focus the exercise on Angular architecture and RxJS data flow  

The service is structured so it can be easily replaced with a real  
`HttpClient` implementation without changing component logic.
